...
import { updateAcUI, updateBUI, updatePcUI, updateVaUI, updateIxUI, updateBpUI, updateVbUI, updateCacheUI, formatNumber, updateTidUI, updateBaseUI } from './ui.js';

// salva o contexto da thread atual antes da troca
...
// restaura o contexto da nova thread que vai executar
...
// alterna para a proxima thread ativa (round-robin)
function switchThread(state) {
    let nextId = (state.currentThreadId + 1) % CONSTANTS.MAX_THREADS;
    let found = false;
    
    // procura a proxima thread ativa
    for(let i = 0; i < CONSTANTS.MAX_THREADS; i++) {
        if(state.threads[nextId].active) {
            found = true;
            break;
        }
        nextId = (nextId + 1) % CONSTANTS.MAX_THREADS;
    }
    if(found) {
...
// executa uma unica instrucao de maquina e gerencia flags
export function executeSingleInstruction(state, ui, memorySize) {
    ...
    switch (instr) {
        ...
        case 'SPAWN': {
            let spawned = false;
            // procura uma thread inativa para instanciar (dentro do limite global)
            for(let i = 1; i < CONSTANTS.MAX_THREADS; i++) {
                if(!state.threads[i].active) {
                    ...
                    state.ac = i; // ac recebe o tid retornado
                    spawned = true;
                    break;
                }
            }
            // se o limite estourar, preenche com 255 em hexadecimal (erro)
            if(!spawned) state.ac = 255;
            state.pc += 2;
            break;
        }
        ...
        case 'JOIN': {
            ...
            // verifica se a thread alvo e valida e ainda esta ativa
            if(targetId >= 0 && targetId < CONSTANTS.MAX_THREADS && state.threads[targetId].active && targetId !== state.currentThreadId) {
                saveContext(state);
                switchThread(state);
                restoreContext(state);
                overrideFlags = true;
        ...
        case 'COREID': {
            // retorna o id do core da thread atual
            state.ac = state.threads[state.currentThreadId].coreId; 
            state.pc++;
            break;
        }
...
// reseta o estado da maquina para condicoes iniciais
export function resetState(state, ui, iCacheSize, dCacheSize, vectorRegSize) {
    state.currentThreadId = 0;
    
    // reinicia o array de threads iterando ate o limite maximo
    state.threads = [];
    for(let i = 0; i < CONSTANTS.MAX_THREADS; i++) {
        state.threads.push({
            active: (i === 0),
            coreId: Math.floor(i / CONSTANTS.THREADS_PER_CORE),
            pc: 0, ac: 0, b: 0,
            va: new Array(vectorRegSize).fill(0),
            ix: 0, bp: 0,
            vb: new Array(vectorRegSize).fill(0),
